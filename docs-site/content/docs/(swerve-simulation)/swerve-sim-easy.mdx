...
To do this, we will use the `SimplifiedSwerveDriveSimulation` class. This class manages and controls a [SwerveDriveSimulation](/swerve-simulation-overview), similar to how your user code controls the physical chassis. Based on the commands sent by the user, it runs closed loops on the virtual drive/steer motors in the simulated drivetrain, mimicking the behavior of the real swerve drive.
