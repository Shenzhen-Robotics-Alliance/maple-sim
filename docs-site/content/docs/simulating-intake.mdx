...
<Callout type="info" title="Tip">
  As shown in the code above, you can notify `FlyWheelIOSim` (the simulated flywheel mechanism) to shoot the note out when the note is passed to the shooter from the intake. See [Simulating GamePiece Projectiles](/simulating-projectiles) for more details.
