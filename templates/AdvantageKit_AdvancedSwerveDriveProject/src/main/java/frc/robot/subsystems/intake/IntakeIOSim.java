...
public class IntakeIOSim extends IntakeSimulation implements IntakeIO {
  ...
  public IntakeIOSim(AbstractDriveTrainSimulation driveTrain, Runnable passNoteToFlyWheelsCall) {
    super( // create intake simulation with no extension
        ...
        IntakeSide.BACK, // the intake is attached the back of the drivetrain
        1 // the intake can only hold 1 game piece at a time
        );

    // register the intake to the default simulation field
    super.register();
  ...
  public void updateInputs(IntakeInputs inputs) {
    ...
    // inputs
    inputs.noteDetected = super.gamePiecesInIntakeCount != 0;

    // if the intake voltage is higher than 2 volts, it is considered running
    if (intakeVoltage > 4) super.startIntake();
    // otherwise, it's stopped
    else super.stopIntake();
    ...
    if (intakeVoltageIntegralSinceNoteTaken < 0) {
      ...
      super.gamePiecesInIntakeCount = 0;
    }
    ...
    // touch the fly wheels
      ...
      // clear the game pieces in this intake
      super.gamePiecesInIntakeCount = 0;
    }
  }
